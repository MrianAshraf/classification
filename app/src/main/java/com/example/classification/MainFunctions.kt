...
class MainFunctions : AppCompatActivity() {
    ...
    var norm = ""
    val listreport = ListReports()
    val addrepp = Addarepp()
    ...
    private fun Lymphocyte(image: Bitmap?): String {
        var res = ""
        try {
            val model: Lymphocyte = Lymphocyte.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Lymphocyte.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            ...
            val classes = arrayOf("Normal", "Abnormal")
    ...
    private fun Basophil(image: Bitmap?): String {
        var res = ""
        try {
            val model: Basophil = Basophil.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Basophil.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            val confidences: FloatArray = outputFeature0.getFloatArray()
            //            // find the index of the class with the biggest confidence.
            var maxPos = 0
    ...
    private fun Eosinophil(image: Bitmap?): String {
        var res = ""
        try {
            val model: Eosinophil = Eosinophil.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Eosinophil.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            val confidences: FloatArray = outputFeature0.getFloatArray()
            //            // find the index of the class with the biggest confidence.
            var maxPos = 0
    ...
    private fun Erythroblast(image: Bitmap?): String {
        var res = ""
        try {
            val model: Erythroblast = Erythroblast.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Erythroblast.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            val confidences: FloatArray = outputFeature0.getFloatArray()
            //            // find the index of the class with the biggest confidence.
            var maxPos = 0
    ...
    private fun monocyte(image: Bitmap?): String {
        var res = ""
        try {
            val model: Monocyte = Monocyte.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Monocyte.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            val confidences: FloatArray = outputFeature0.getFloatArray()
            //            // find the index of the class with the biggest confidence.
            var maxPos = 0
    ...
    private fun Neutrophil(image: Bitmap?): String {
        var res = ""
        try {
            val model: Neutrophil = Neutrophil.newInstance(applicationContext)
            ...
            // Runs model inference and gets result.
            val outputs: Neutrophil.Outputs = model.process(inputFeature0)
            val outputFeature0: TensorBuffer = outputs.getOutputFeature0AsTensorBuffer()
            val confidences: FloatArray = outputFeature0.getFloatArray()
            //            // find the index of the class with the biggest confidence.
            var maxPos = 0
