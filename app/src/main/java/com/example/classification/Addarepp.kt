...
//                insertrepo(name, state, pphone, dcomment, age)
    ...
    private fun insertrepo(name: String, state: String,phone:String,comm:String,age:Int) {
        val patient = Patientclass(
            patientage =age,
        patientname = name,
        patientphone = phone,
        patientstate =state,
        doctorcomment =comm)
        Mydatabase.getinstance(this).gpreportdao().addpatient(patient)
        Toast.makeText(this,"Todo Is Added Successfully", Toast.LENGTH_SHORT).show()
        //dismiss()
        onAddclick?.onaddclick()
